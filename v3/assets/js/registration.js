...
const Registration = {
    ...
    async loadEventClasses() {
        try {
            const response = await fetch(`/v3/api/registration.php?action=event_classes&event_id=${this.eventId}`);
    ...
    async addExistingRider(riderId) {
        ...
        try {
            const response = await fetch(
                `/v3/api/registration.php?action=get_rider&rider_id=${riderId}&event_id=${this.eventId}`
            );
            const data = await response.json();
