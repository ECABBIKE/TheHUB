...
<main class="main-content">
 ...
 <div class="license-card-container">
  ...
  <div class="license-content" style="flex-direction: column; align-items: center; text-align: center;">
   ...
   <!-- Location -->
   <?php if ($club['city']): ?>
    <div class="license-id" style="color: var(--primary); font-weight: 600;">
   ...
   <?php if (!empty($club['description'])): ?>
    <div class="text-secondary mt-md" style="max-width: 500px; margin: 0 auto; font-size: 0.875rem;">
    <?= nl2br(h($club['description'])) ?>
    </div>
   <?php endif; ?>

   <!-- Stats Grid -->
   <div class="info-grid-compact mt-lg" style="grid-template-columns: repeat(4, 1fr);">
    <div class="info-box">
   ...
   <div class="mt-lg" style="background: rgba(0,0,0,0.03); padding: 1rem; border-radius: 8px;">
    <div class="flex gap-md flex-wrap" style="justify-content: center;">
    <?php if (!empty($club['website'])): ?>
    <a href="<?= h($club['website']) ?>" target="_blank" rel="noopener" class="flex items-center gap-xs link">
    ...
    <a href="mailto:<?= h($club['email']) ?>" class="flex items-center gap-xs link">
    ...
    <a href="tel:<?= h($club['phone']) ?>" class="flex items-center gap-xs link">
    ...
    <a href="<?= h($club['facebook']) ?>" target="_blank" rel="noopener" class="flex items-center gap-xs link">
    ...
    <a href="<?= h($club['instagram']) ?>" target="_blank" rel="noopener" class="flex items-center gap-xs link">
    ...
    <div class="text-center mt-sm text-xs text-secondary">
 ...
 <div class="gs-tabs mb-lg">
  <a href="?id=<?= $clubId ?>&tab=medlemmar" class="tab <?= $tab === 'medlemmar' ? 'active' : '' ?>">
  <i data-lucide="users"></i>
  Medlemmar
  </a>
  <a href="?id=<?= $clubId ?>&tab=ranking&discipline=<?= $discipline ?>" class="tab <?= $tab === 'ranking' ? 'active' : '' ?>">
 ...
 <div class="card">
  ...
  <?php if (empty($clubRiders)): ?>
   <div class="text-center gs-empty-state-container">
   <i data-lucide="users" class="gs-empty-state-icon-lg"></i>
   <h3 class="mb-sm">Inga medlemmar hittades</h3>
   <p class="text-secondary">
    Denna klubb har inga aktiva medlemmar registrerade.
   </p>
   </div>
  <?php else: ?>
   <div class="grid grid-cols-1 md-grid-cols-2 gs-lg-grid-cols-3 gap-md">
   <?php foreach ($clubRiders as $rider): ?>
    ...
    <?php if ($riderClass): ?>
     <div class="badge badge-primary badge-sm gs-mb-2">
     <?= h($riderClass) ?>
     </div>
    <?php endif; ?>
 ...
 <div class="card">
  ...
  <h2 class="text-primary flex items-center gap-sm">
   <i data-lucide="trophy"></i>
   Ranking-bidrag
  </h2>
  ...
  <div class="gs-ml-auto">
   <select class="input input-sm" onchange="window.location.href='?id=<?= $clubId ?>&tab=ranking&discipline=' + this.value">
  ...
  <?php if (empty($rankingContributors)): ?>
   ...
   <h3 class="mb-sm">Inga rankingpoäng ännu</h3>
   <p class="text-secondary">
  ...
  <?php else: ?>
   <div class="gs-ranking-info-banner mb-lg" style="display: flex; align-items: flex-start; gap: 0.75rem; padding: 1rem; background: #E8F4FD; border-radius: 8px; font-size: 0.875rem;">
   ...
   <table class="table">
    ...
    <tr>
     ...
     <th class="text-right">Rankingpoäng</th>
    </tr>
    </thead>
    <tbody>
    <?php $position = 1; ?>
    <?php foreach ($rankingContributors as $contributor): ?>
     ...
     <td class="text-center"><?= $contributor['events_count'] ?></td>
     <td class="text-right">
      <strong class="text-primary"><?= number_format($contributor['total_ranking_points'], 1) ?>p</strong>
     </td>
     </tr>
     <?php $position++; ?>
    <?php endforeach; ?>
    ...
    <tr style="background: #f8f9fa; font-weight: bold;">
     <td colspan="3" class="text-right">Total klubbpoäng:</td>
     <td class="text-right">
