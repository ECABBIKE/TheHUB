 ...
 * OBS: Vi letar bland ALLA riders, även de med UCI-ID!
 * Vi vill hitta dubletter oavsett om de har licensnummer eller inte.
 * 
 * EXEMPEL:
 * ID 10624: "Milton GRUNDBERG" (ingen UCI-ID)
 * ID 9460:  "Milton Jonsson Grundberg" (har UCI-ID)
 * → De ska matchas eftersom de har samma förnamn + gemensamt efternamn!
 */
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <?php if ($masterRider === 1): ?>
                                                <br><span class="gs-badge gs-badge-success" style="font-size: 11px; margin-top: 4px;">✓ BEHÅLLS</span>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td>
                                            <?php
                                                // Prioritet 1: Den med UCI-ID blir master
                                                // Prioritet 2: Annars den med flest resultat
                                                ...
                                                if ($rider1HasUCI && !$rider2HasUCI) {
                                                  // Rider 1 har UCI, Rider 2 inte
                                                  $keepId = $dup['rider1_id'];
                                                  $mergeId = $dup['rider2_id'];
                                                } elseif ($rider2HasUCI && !$rider1HasUCI) {
                                                  // Rider 2 har UCI, Rider 1 inte
                                                  $keepId = $dup['rider2_id'];
                                                  $mergeId = $dup['rider1_id'];
                                                } else {
                                                  // Båda eller ingen har UCI - behåll den med flest resultat
                                            ...
                                            <div class="gs-flex gs-gap-xs">
                                                <form method="POST" class="gs-inline" onsubmit="return confirm('Sammanfoga denna? Alla resultat sparas i ' + (<?= $masterRider ?> === 1 ? '<?= h($dup['rider1_name']) ?>' : '<?= h($dup['rider2_name']) ?>') + ' (ID ' + (<?= $masterRider ?> === 1 ? <?= $dup['rider1_id'] ?> : <?= $dup['rider2_id'] ?>) + ').');">
                                                    <?= csrf_field() ?>
