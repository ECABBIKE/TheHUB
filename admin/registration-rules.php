...
?>
 ...
 <div class="card-body">
 <form method="GET" class="flex gs-items-end gap-md">
  <div class="form-group flex-1">
  <label for="series_id" class="label">Serie</label>
  <select name="series_id" id="series_id" class="input" onchange="this.form.submit()">
 ...
 <div class="alert alert--info mb-lg">
  ...
  ?>
  <br><span class="text-secondary"><?= h($ruleTypeInfo['description']) ?></span>
  <?php endif; ?>
 </div>
 <?php endif; ?>
 ...
 <a href="?series_id=<?= $selectedSeriesId ?>&view=series"
  class="btn <?= $view === 'series' ? 'btn--primary' : 'btn--secondary' ?>">
  <i data-lucide="list"></i>
  Klassregler (<?= count($seriesClassRules) ?>)
 </a>
 <a href="?series_id=<?= $selectedSeriesId ?>&view=events"
  class="btn <?= $view === 'events' ? 'btn--primary' : 'btn--secondary' ?>">
 ...
 <div class="card">
  ...
  <button type="button" class="btn btn--primary btn--sm" onclick="openAddClassRuleModal('series', <?= $selectedSeriesId ?>)">
  ...
  <table class="table">
   ...
   <td>
    ...
    echo '<span class="badge">Alla</span>';
    } else {
    foreach ($genders as $g) {
    $label = $g === 'M' ? 'Herrar' : ($g === 'K' ? 'Damer' : $g);
    echo '<span class="badge badge-' . ($g === 'M' ? 'info' : 'accent') . '">' . h($label) . '</span> ';
    }
    }
    ?>
   </td>
   ...
   <td>
    ...
    if (empty($licenses)) {
    echo '<span class="text-secondary">Alla</span>';
    } else {
   ...
   <td>
    ...
    <span class="badge badge-warning">Licens</span>
    <?php endif; ?>
    <?php if ($rule['requires_club_membership']): ?>
    <span class="badge badge-info">Klubb</span>
    <?php endif; ?>
   </td>
   <td>
   ...
   <td>
    ...
    <button type="submit" class="btn btn--secondary btn--sm text-error">
 ...
 <div class="card">
  ...
  <p class="text-secondary">Inga events finns i denna serie.</p>
  <?php else: ?>
  <div class="table-responsive">
  <table class="table">
   <thead>
   ...
   <td>
    ...
    <span class="badge badge-info">Från serie</span>
    <?php else: ?>
    <span class="badge badge-warning">Eventspecifik</span>
    <?php endif; ?>
   </td>
   <td>
...
<div id="classRuleModal" class="gs-modal hidden">
 ...
 <div class="grid grid-cols-1 md-grid-cols-2 gap-lg">
  ...
  <i data-lucide="users"></i>
  Klass <span class="text-error">*</span>
  </label>
  <select name="class_id" id="classRuleClassId" class="input" required>
  ...
  <div>
  <label for="min_age" class="text-sm">Min ålder</label>
  <input type="number" name="min_age" id="classRuleMinAge" class="input" min="0" max="99">
  </div>
  <div>
  <label for="max_age" class="text-sm">Max ålder</label>
  <input type="number" name="max_age" id="classRuleMaxAge" class="input" min="0" max="99">
  </div>
  </div>
  </div>

  <!-- License Types -->
  <div class="form-group gs-md-col-span-2">
  <label class="label">
  <i data-lucide="badge"></i>
  Tillåtna licenstyper
  </label>
  <div class="grid grid-cols-2 gs-md-grid-cols-4 gap-sm">
 ...
 <button type="button" class="btn btn--secondary" onclick="closeClassRuleModal()">
  Avbryt
 </button>
 <button type="submit" class="btn btn--primary">
  <i data-lucide="save"></i>
...
<div id="eventRuleModal" class="gs-modal hidden">
 ...
 <div class="gs-modal-body">
 <div class="form-group">
  <label class="label">Regelkälla</label>
  <div class="flex flex-col gap-sm">
  <label class="flex items-center gap-sm">
 ...
 <div id="eventRuleTypeSection" class="form-group hidden">
  <label for="event_rule_type_id" class="label">
  Regeltyp för eventet
  </label>
  <select name="event_rule_type_id" id="eventRuleTypeId" class="input">
 ...
 <div class="gs-modal-footer">
 <button type="button" class="btn btn--secondary" onclick="closeEventRuleModal()">
  Avbryt
 </button>
 <button type="submit" class="btn btn--primary">
  <i data-lucide="save"></i>
...
<!-- Preview Modal -->
<div id="previewModal" class="gs-modal hidden">
 ...
 <p class="text-center text-secondary">Laddar...</p>
 </div>
 </div>
 <div class="gs-modal-footer">
...
async function previewClassRiders(seriesId, classId) {
 ...
 content.innerHTML = '<p class="text-center text-secondary">Laddar...</p>';
 ...
 if (events.length === 0) {
 content.innerHTML = '<p class="text-error">Inga events finns i serien.</p>';
 return;
 }
 ...
 <div class="mb-md">
  ...
  <span class="text-success"><strong>Behöriga:</strong> ${data.eligible_count}</span><br>
  <span class="text-error"><strong>Ej behöriga:</strong> ${data.ineligible_count}</span>
 </div>
 `;

 if (data.eligible.length > 0) {
 html += '<h4 class="mb-sm">Behöriga åkare</h4>';
 ...
 data.eligible.forEach(rider => {
  ...
  html += ` <span class="text-warning text-sm">⚠ ${rider.warnings.join(', ')}</span>`;
  }
  html += '</li>';
 });
 html += '</ul>';
 ...
 html += '<h4 class="mt-lg mb-sm">Ej behöriga</h4>';
 html += '<ul class="gs-list">';
 data.ineligible.forEach(rider => {
  html += `<li class="text-error">${rider.name} - ${rider.errors.join(', ')}</li>`;
 });
 html += '</ul>';
 }
 ...
 content.innerHTML = `<p class="text-error">${data.error || 'Ett fel uppstod'}</p>`;
 }
 } catch (error) {
 content.innerHTML = `<p class="text-error">Kunde inte ladda data: ${error.message}</p>`;
 }
}
