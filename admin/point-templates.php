  ...
  <form method="POST" id="templateForm">
   ...
   <div class="grid grid-cols-1 gap-md">
    ...
    <div class="flex items-center justify-between mb-sm">
     <label class="label gs-margin-0">Poängfördelning per placering</label>
     <button type="button" class="btn btn-xs btn--secondary" onclick="addPointRow()">
     <i data-lucide="plus" class="gs-icon-12"></i>
     Lägg till rad
     </button>
    </div>
    <p class="text-xs text-secondary mb-sm">Klicka på"Lägg till rad" för att lägga till fler placeringar</p>
...
3,60'></textarea>
   ...
   <div class="alert alert--info">
    <p class="text-xs"><strong>JSON format:</strong> Klistra in exporterad JSON-data</p>
    <p class="text-xs"><strong>CSV format:</strong> En rad per placering: placering,poäng</p>
   </div>
   </div>
 ...
 <div class="card">
  ...
  <?php else: ?>
   ...
   ?>
   <div class="card mb-md gs-bg-secondary">
    <div class="card-body">
    <div class="flex items-start justify-between mb-sm">
     <div>
     <h3 class="text-primary gs-margin-0-0-qtr-0">
      <?= h($template['name']) ?>
     </h3>
     <?php if (!empty($template['description'])): ?>
      <p class="text-sm text-secondary"><?= h($template['description']) ?></p>
     <?php endif; ?>
     </div>
     <div class="flex gap-sm">
     <button type="button"
    ...
    <div class="text-xs text-secondary mt-md">
     <strong>Poäng:</strong>
     <?php if (empty($points)): ?>
     <span>Ingen poängfördelning angiven</span>
     <?php else: ?>
     <div class="flex gap-xs flex-wrap gs-mt-xs">
      <?php foreach ($points as $position => $pointValue): ?>
      <span class="badge badge-sm gs-bg-badge-neutral">
 ...
 function addPointRow(position = null, points = '') {
  ...
  row.className = 'flex items-center gap-sm gs-mb-xs';
  row.innerHTML = `
  <div class="gs-point-row-label-wrapper">
   <label class="text-xs text-secondary">Plats #${nextPosition}</label>
  </div>
  ...
  <button type="button" class="btn btn-xs btn--secondary btn-danger" onclick="this.parentElement.remove()">
