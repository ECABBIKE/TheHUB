 ...
 * Supports both editing existing destinations and creating new ones
 * (Uses venues table internally)
 */
...
if (!$isNew) {
    ...
    if (!$existingVenue) {
        $_SESSION['message'] = 'Destination hittades inte';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete' && !$isNew) {
        ...
        if ($eventCount > 0) {
            $message = "Kan inte ta bort destination med $eventCount kopplade events. Flytta eventen forst.";
            $messageType = 'error';
        } else {
            try {
                $db->delete('venues', 'id = ?', [$id]);
                header('Location: /admin/destinations.php?msg=deleted');
                exit;
            } catch (Exception $e) {
        ...
        if (empty($name)) {
            $message = 'Namn pa destination ar obligatoriskt';
            $messageType = 'error';
        } else {
            // Prepare venue data
            $venueData = [
            ...
            try {
                if ($isNew) {
                    ...
                    $id = $db->lastInsertId();
                    header('Location: /admin/destination-edit.php?id=' . $id . '&msg=created');
                    exit;
                } else {
...
$page_title = $isNew ? 'Ny Destination' : 'Redigera Destination';
$breadcrumbs = [
    ['label' => 'Destinations', 'url' => '/admin/destinations.php'],
    ['label' => $isNew ? 'Ny Destination' : h($venue['name'])]
];
...
<form method="POST">
  ...
  <div>
   ...
   <div class="card-body">
    ...
    <?= $isNew ? 'Skapa destination' : 'Spara andringar' ?>
    </button>
    <a href="/admin/destinations.php" class="btn btn--secondary w-full">
   ...
   <div class="card-body">
    ...
    <p class="text-secondary text-sm mb-md">
     Destinationen har <?= $eventCount ?> kopplade events och kan inte tas bort.
    </p>
    ...
    <button type="submit" name="action" value="delete" class="btn btn-danger w-full"
     onclick="return confirm('Ar du saker pa att du vill ta bort denna destination?')">
     <i data-lucide="trash-2"></i>
     Ta bort destination
    </button>
    <?php endif; ?>
...
<div class="card mt-lg">
    <div class="card-header flex justify-between items-center">
        <h2>
            ...
            Events pa denna destination (<?= $eventCount ?>)
        </h2>
